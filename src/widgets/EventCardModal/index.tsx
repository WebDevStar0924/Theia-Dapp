...
export default function EventCardModal(props: iProps) {
  const [showAttendModal, setShowAttendModal] = useState<boolean>(false);


  return (
    <ModalProvider >
      <Flex flexDirection={"row"} style={{ gap: "30px" }}>
        <Modal
          title={""}
        ...
        >
          ...
          <EventCardModalBodyWrapper>
            <Flex flexDirection={"column"} style={{ gap: "17px", marginTop: "8px" }}>
              ...
              <Flex className={"addToCalendarButton"} style={{ margin: "32px 40px 0px 40px" }} onClick={() => setShowAttendModal(true)}>
                ATTEND
              </Flex>
            </Flex>
          </EventCardModalBodyWrapper>
        </Modal>
        {showAttendModal && <AttendEventModal></AttendEventModal>}
      </Flex>
    </ModalProvider >
  );
}
