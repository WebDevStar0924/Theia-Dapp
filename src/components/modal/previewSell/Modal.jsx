...
const Modal = ({ previewSellModal, setPreviewSellModal }) => {
  const modalRef = useRef()
  const closeModal = (e) => {
    if (modalRef.current === e.target) {
      setPreviewSellModal(false)
    }
  }
  ...
  const keyPress = useCallback(
    (e) => {
      if (e.key === 'Escape' && previewSellModal) {
        setPreviewSellModal(false)
      }
    },
    [setPreviewSellModal, previewSellModal],
  )
        ...
        >
          <motion.div variants={modal} className="modal">
            <div className="modal-title-container">
              <div className="modal-title">I want to sell</div>
              <div className="modal-subtitle">Avalable: 3,829,000 TEA </div>
            </div>
            <div className="modal-input-container">
              <input className="modal-input" />
              <div className="modal-input-right">
                <p className="modal-input-p" style={{ marginRight: '10px' }}>
                  Max
                </p>
